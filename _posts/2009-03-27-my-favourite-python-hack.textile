...
* Replace it with __magic_var__ and add this variable to the locals dictionary
* When the user "drags" the number, replace the value of __magic_var__ with an updated value and run the script again.
...
Note that we replaced the original number with the name __magic_var__ in our original source code, and compiled that. After compilation succeeds and returns an AST, a function called _checkSigns recursively checks the AST for special sign cases. First, it tries to find the magic variable. Once it has found that, it checks the parent node to see if it encounters any of the special cases, and basically prepares the object for string replacement afterwards.

What I like about this hack is that if it works correctly, you don't even notice it's there.

If you'd like to view the code, it's in the NodeBox source as "ValueLadder.py":http://dev.nodebox.net/browser/nodebox/trunk/src/nodebox/gui/mac/ValueLadder.py.