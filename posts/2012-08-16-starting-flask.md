...
One server can host multiple running web applications, and they probably all use different versions of the underlying frameworks. To manage these dependencies, we'll create a _virtual environment_ where we install the dependencies locally, together with the project instead of globally. In Python, we'll do this using virtualenv and pip. These tools provide a clean way to manage the dependencies of different Python projects.
...
Each time you work on this project, you need to _activate_ the virtual environment. From a new Terminal window:
...
When deploying to a webserver it is important to register which requirements we need. To do this we _freeze_ the installed packages and store this setup in a `requirements.txt` file:
...
[Fabric](http://docs.fabfile.org/) is a command-line utility that runs a `fabfile` containing instructions on how to deploy to a server. It can automate _any_ server task, not just deployment.
