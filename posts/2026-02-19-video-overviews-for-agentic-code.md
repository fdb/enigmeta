...
As said above we use the [Remotion](https://www.remotion.dev/) framework to build the videos. The nice thing is that these are React components that can be assembled into slides. Each visual element — code blocks, waveform visualizers, architecture diagrams — is a component that animates based on frame count.

I ask Claude Code to generate a version with "burned in" captions first, then when the flow looks correct, I ask it to generate narration through ElevenLabs text-to-speech. The resulting audio durations determine how long each visual section lasts.
...
**"Diving back in"** — You took a vacation, or switched to another project for two weeks. Ask the AI to generate a just-in-time video to catch you up on what happened while you were away.
...
The scaffolding is there, though. A narration-driven explaination system could work quite well, and is not specific to any particular type of code. The pattern of rough structure → fine-grained scene plan → transcript → scenes → TTS + retiming works quite well.
